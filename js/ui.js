...
function attachDrag(card, restaurant) {
  let startX = 0, startY = 0, dx = 0, dy = 0;
  const ROTATION_FACTOR = 0.1;
  let isDragging = false;

  console.log('[UI] attachDrag - restaurant:', restaurant.name, 'id:', restaurant.id);
  ...
  const flyOut = (liked) => {
    ...
    setTimeout(() => {
      console.log('[UI] Calling handleCardSwipe with restaurant:', restaurant.name);
